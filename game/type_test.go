...
func TestType_String(t *testing.T) {
	...
	if !(test.IsStringer(r) && test.IsStringer(b) && test.IsStringer(n)) {
		t.Errorf("Type must implement `Stringer` interface")
		return
	}

	if r.String() == b.String() || b.String() == n.String() || n.String() == r.String() {
		t.Errorf("Type string output must be unique for `RED` (%q), `BLUE` (%q), and `NONE` (%q)", r, b, n)
	}
}
...
func TestType_newRandType(t *testing.T) {
	const (
		...
		// the probability of a random output being falsely identified as non-random is around 0.002%.
	...
	if minPass := int(math.Floor(ROUNDS / 3 * (1 - TOLERANCE))); red < minPass || blue < minPass || none < minPass {
		t.Errorf("newRandType must give random mix of `RED`s, `BLUE`s, and `NONE`s. Probabilistic, try test again")
	}
}
